...
try:
    ...
    print("[BlenderTalk] Installing websockets package...")
    ...
    for node in world.node_tree.nodes:
        if node.type == 'TEX_ENVIRONMENT' and node.image:
            ...
            if not _ensure_image_loaded(img):
                print(f"[BlenderTalk] WARNING: could not load environment image '{img.name}'")
                continue
            w, h = img.size
            if w == 0 or h == 0:
                print(f"[BlenderTalk] WARNING: environment image '{img.name}' has zero size")
                continue
            ...
            if hdr_bytes is None and img.filepath:
                try:
                    ...
                    print(f"[BlenderTalk] WARNING: could not read environment file '{img.filepath}': {e}")
                    continue
            if not hdr_bytes:
                print(f"[BlenderTalk] WARNING: no data for environment image '{img.name}'")
                continue
            ...
            print(f"[BlenderTalk] HDR blob sent: {img.name} ({w}×{h}, {len(hdr_bytes)} bytes)")
            return result
        ...
        for node in mat.node_tree.nodes:
            ...
            for input_name in INPUTS:
                ...
                if img_bytes is None:
                    print(f"[BlenderTalk] WARNING: no image data for '{name}'")
                    continue
                ...
                with _lock:
                    ...
                    for ws_set in _state["tex_sent"].values():
                        ws_set.discard(name)
                print(f"[BlenderTalk] Texture extracted: {name} ({w}×{h}, {ext})")
            break
        ...
        for area in scr.areas:
            ...
            try:
                with bpy.context.temp_override(window=window, area=area):
                    ...
                    print(f"[BlenderTalk] Viewport camera synced — fov={fov:.1f}°")
                    return {
                        "type": "camera",
                ...
                print(f"[BlenderTalk] Camera context override failed: {e}")
                continue

    print("[BlenderTalk] Camera: no 3D viewport found")
    return None
...
async def _handler(websocket):
    ...
    print(f"[BlenderTalk] Client connected ({len(_state['clients'])} total)")
    try:
        ...
        print(f"[BlenderTalk] Client disconnected ({len(_state['clients'])} total)")
...
async def _run_server(port: int):
    ...
    async with websockets.serve(_handler, SERVER_HOST, port) as server:
        ...
        _set_running(True, "")
        print(f"[BlenderTalk] Server listening on ws://{SERVER_HOST}:{port}")
    ...
    with _lock:
        _state["server"] = None
    print("[BlenderTalk] Server shut down cleanly.")
...
def _server_thread(port: int):
    ...
    try:
        ...
        print(f"[BlenderTalk] ERROR: {msg} ({e})")
        _set_running(False, msg)
    except Exception as e:
        msg = f"Server error: {e}"
        print(f"[BlenderTalk] ERROR: {msg}")
        _set_running(False, msg)
    finally:
        _set_running(False, "")
        loop.close()
        with _lock:
            if _state["loop"] is loop:
                _state["loop"] = None
        print("[BlenderTalk] Event loop closed.")


# ---------------------------------------------------------------------------
# Timer (fires at ~30 Hz from Blender's main thread)
# ---------------------------------------------------------------------------
def _timer():
    """Blender timer — sends scene data and HDR (~30 Hz), keeps panel in sync."""
    try:
        with _lock:
            loop = _state["loop"]
        ...
        if loop is not None and not loop.is_closed() and current:
            ...
            # --- Camera data (JSON) — viewport at 30 Hz, scene cameras on change ---
            with _lock:
                send_cam = _state["camera_sync_enabled"]
            if send_cam:
                # Viewport camera — send every frame (~30 Hz), unchanged
        ...
        import traceback
        print(f"[BlenderTalk] ERROR in timer: {e}")
        traceback.print_exc()
    ...
    try:
        ...
        for line in result.stdout.strip().splitlines():
            ...
            try:
                ...
                killed.append(pid)
                print(f"[BlenderTalk] Killed PID {pid} holding port {port}")
            except (ProcessLookupError, PermissionError) as e:
                print(f"[BlenderTalk] Could not kill PID {pid}: {e}")
        ...
        if isinstance(e, FileNotFoundError):
            print("[BlenderTalk] lsof not found — cannot check port usage")
        else:
            print("[BlenderTalk] lsof timed out — skipping port check")
...
def start_server():
    ...
    t = threading.Thread(target=_server_thread, args=(port,), daemon=True, name="blendertalk-ws")
    with _lock:
        _state["thread"] = t
        _state["port"] = port
    t.start()
...
def stop_server():
    ...
    if old_thread is not None and old_thread.is_alive():
        ...
        if old_thread.is_alive():
            print("[BlenderTalk] NOTE: server thread still exiting…")


# ---------------------------------------------------------------------------
# Blender operators
# ---------------------------------------------------------------------------
class BLENDERTALK_OT_start(bpy.types.Operator):
    bl_idname = "blendertalk.start"
...
class BLENDERTALK_OT_stop(bpy.types.Operator):
    bl_idname = "blendertalk.stop"
...
# ---------------------------------------------------------------------------
# Blender panel (3D View → Sidebar → BlenderTalk tab)
# ---------------------------------------------------------------------------
class BLENDERTALK_PT_panel(bpy.types.Panel):
    bl_label = "BlenderTalk Server"
    bl_idname = "BLENDERTALK_PT_panel"
    bl_space_type = 'VIEW_3D'
    bl_region_type = 'UI'
    bl_category = "BlenderTalk"
    ...
    def draw(self, context):
        ...
        row.label(text="Port")
        row.prop(scene, "blendertalk_port", text="")
        ...
        # Buttons
        if running:
            layout.operator("blendertalk.stop", icon='PAUSE')
        else:
            layout.operator("blendertalk.start", icon='PLAY')
...
# ---------------------------------------------------------------------------
CLASSES = (
    BLENDERTALK_OT_start,
    BLENDERTALK_OT_stop,
    BLENDERTALK_PT_panel,
)
...
def register():
    ...
    bpy.types.Scene.blendertalk_port = bpy.props.IntProperty(
        name="Port",
